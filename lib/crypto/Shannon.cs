...
namespace lib.crypto
{
    ...
    /// <remarks>Felix Bruns (felixbruns@web.de)</remarks>
    public class Shannon
    {
        ...
        private static int FOLD = N;

        /// How many iterations of folding to do. ///
        private static int INITKONST = 0x6996c53a;

        /// Value of konst to use during key loading. ///
        private static int KEYP = 13;
        ...
        public void stream(byte[] buffer)
        {
            ...
            {
                buffer[i++] ^= Convert.ToByte(this.sbuf & 0xFF);
            ...
            while (i < j)
            {
                ...
                /// XOR word. ///
                buffer[i + 3] ^= Convert.ToByte((this.sbuf >> 24) & 0xFF);
                buffer[i + 2] ^= Convert.ToByte((this.sbuf >> 16) & 0xFF);
                buffer[i + 1] ^= Convert.ToByte((this.sbuf >> 8) & 0xFF);
                buffer[i] ^= Convert.ToByte((this.sbuf) & 0xFF);
        ...
        public void encrypt(byte[] buffer, int n)
        {
            ...
            if (this.nbuf != 0)
            {
                while (this.nbuf != 0 && n != 0)
                {
                    ...
                    buffer[i] ^= Convert.ToByte((this.sbuf >> (32 - this.nbuf)) & 0xFF);
