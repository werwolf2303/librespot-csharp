...
namespace lib.audio.cdn
{
    /*public class CdnFeedHelper
    {
        ...
        {
            return new Uri(resp.Cdnurls[session.random().Next(0, resp.Cdnurls.Count)]);
        }
        
        public static LoadedStream loadTrack(@NotNull Session session, Metadata.@NotNull Track track, Metadata.@NotNull AudioFile file,
